...
def plot_dist(Z, Y, n_example=0):
    tsne = TSNE(random_state=348, init="pca")
    z_o = tsne.fit_transform(Z, Y)
    fig = plt.figure(figsize=(8,8))
    ax = fig.add_subplot(111)
    ax.scatter(z_o[:,0], z_o[:,1], c=Y, s=5, marker="^", cmap="tab20")
    fig.savefig("plots/dist_{}.eps".format(n_example), dpi=300, figsize=(8,8), bbox_inches="tight")
...
def check_dist_space(X, Y, args, n_classes=10, exp=0, split=0):
    ...
    with tf.Graph().as_default():
        ...
        iterador = dset.create_eval_generator()
        x_input, y_input = iterador.get_next()
        z_hat, _ = aae.encoder(x_input) #Nos interesa el espacio de distribución
...
def main(args):
    ...
    with open("res_exp3.txt", "w") as f:
        for exp in np.arange(1, args.n_exps +1):
            ...
            f.write("--" + eval_str)
            check_dist_space(Xitest, one_hot(Yitest, args.starting), args, n_classes=args.starting, exp=exp, split=0)
            ...
            for idx, unk in enumerate(unknown_classes):
                ...
                check_dist_space(Xitest, one_hot(Yitest, nclasses), args, n_classes=nclasses, exp=exp, split=idx+1)
